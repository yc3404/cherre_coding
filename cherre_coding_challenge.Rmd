...
```{r}
datatable(as.data.frame(tbl(testdb,sql("SELECT id, count(*) from sites group by id having count(*) > 1"))))
```

### Checking duplicates in people table

```{r}
datatable(as.data.frame(tbl(testdb,sql("SELECT id, count(*) from people group by first_name,last_name having count(*) > 1"))))
```
...
We can see the sites table does not contain duplicates, but the people table contains duplicates. The person with personid 30 has two personids. We need to find the name of this person and change his/her another personid to 30.
